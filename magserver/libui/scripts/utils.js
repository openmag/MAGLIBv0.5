...
function getPopupContainer(obj) {
	...
	var tbl = newTableElement('', 0, 0, 0, '', 1, 2, 'center', 'top');
...
function showDropDownMenu(obj, items) {
	...
	var menu_pane = newTableElement('', 0, 2, 1, '#FFFFFF', items.length, 1, 'left', 'middle');
	container.appendChild(menu_pane);
	menu_pane.style.border = '1px solid #BBBBBB';
	menu_pane.style.fontSize = '12px';
...
function _prepare_config_workingarea(panel) {
	...
	var working_area = newStylishPanel(new_panel, 2);
...
function makeContentPanel(pane, conf) {
	...
	if(conf.menus && conf.menus.length > 0) {
		...
		for(var i = 0; i < conf.menus.length; i ++) {
			...
			if('string' === typeof(conf.menus[i])) {
				...
				tblCell(menu_tbl, 0, i).style.paddingLeft = '20px';
				tblCell(menu_tbl, 0, i).style.paddingRight = '10px';
			}
	...
	var refresh_icon = newImageElement('', IMAGE_ROOT + '/refresh.png', 10, 10, 0, '刷新面板');
...
function newDBGrid(records, conf, width, panel, panel_conf, style) {
	var order_col_bg = '#DDDDDD';
        var tbl = newTableElement(width, 0, 2, 2, "", records.length + 1, conf.length, "", "");
        if(panel != null) {
                panel.appendChild(tbl);
        }
	var order_index = -1;
