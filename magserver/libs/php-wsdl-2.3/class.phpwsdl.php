...
class PhpWsdl{
	...
	public function GetCacheFileName($endpoint=null){
		$data=Array(
			'server'		=>	$this,
			'endpoint'		=>	$endpoint,
			'filename'		=>	(is_null(self::$CacheFolder))?null:self::$CacheFolder.DIRECTORY_SEPARATOR.sha1((is_null($endpoint))?$this->EndPoint:$endpoint).'.wsdl'
		);
		self::CallHook(
			'CacheFileNameHook',
			$data
	...
	public function WriteWsdlToCache($wsdl=null,$endpoint=null,$file=null,$force=false){
		...
		if($temp!='/' && $temp!='.'){
			...
			$file=self::$CacheFolder.DIRECTORY_SEPARATOR.$file;
		}*/
		if(!$force)
			if($this->IsCacheValid($file)){
